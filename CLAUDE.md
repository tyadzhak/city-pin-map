...
│   ├── map-labels.js   # DOM overlay pin labels (any system font/italic): computeLabelSpecs SSOT, Pointer-Events label drag, factory for the inset's labels (interactive/draggable, same store path); mirrored by export.js's paintPinLabels
│   ├── map-inset.js    # Live WYSIWYG corner inset: second MapLibre map fitted to a group's pins + main-map locator layer; draggable box (freePos), frame-aware docking, getResolvedPlacement() the single placement truth; mirrored by export.js's paintInset
