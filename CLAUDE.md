...
Dev-only tooling — it does NOT add an app build step; the app still runs by opening `index.html`. `package.json` is a `devDependencies`-only manifest (sole dep: `c8`). Tests are `node:test` files co-located as `js/*.test.mjs`, run via `npm test`. `npm run coverage` runs the suite under `c8`. No jsdom — shared shims are `js/test-helpers.mjs` (localStorage/document/fetch/timer stand-ins) and `js/xml-shim.mjs` (DOMParser/XMLSerializer stand-in). `.github/workflows/coverage.yml` runs `npm ci` → `npm run coverage` on every push/PR (Node 22) and fails the job under the gate.

**Coverage is scoped to the logic layer ONLY — it is not whole-repo coverage.** The gate enforces ≥80% aggregate **line** coverage over 11 pure/logic modules (`storage`, `svg-ingest`, `import-foreign`, `pins`, `groups`, `settings`, `user-icons`, `icons`, `geocode`, `backup`, `search`), which currently sit at ~99.7% aggregate. The other 13 modules (~6,300 lines) are **excluded from measurement and are at 0%**: `map.js`, `export.js`, `app.js`, `icon-picker.js`, `style-picker.js`, `map-frame.js`, `map-title.js`, `pin-list.js`, `group-panel.js`, `settings-panel.js`, `map-viewport.js`, `map-fade.js`, `side-tabs.js` — they only run in a real browser with MapLibre/WebGL/DOM and are not node-testable without a headless-browser coverage harness (deliberately not adopted). **Whole-repo line coverage is therefore ~33%**, even though the gated logic layer is ~99.7%. Raising the whole-repo number would require browser-driven coverage (e.g. Playwright + V8 instrumentation).
